...
static std::unordered_map<pid_t, struct pid_info> pool;
...
void usched_entry(struct core_info *_core_map)
{
    ...
    for (auto it = pool.begin(); it != pool.end(); ++it) {
        upsert_to_monitor_pool(it->second.ppid, it->second._thread_id,
                               &it->second, _core_map);
...
pid_t usched_check(struct core_info *_core_map, struct pid_info *_pid_info)
{
    printf("pid %d : Thread %d  -> Core %d at %.2f%% utime %lld stime %lld\n",
           _pid_info->ppid, _pid_info->_thread_id, _pid_info->last_cpu,
           _pid_info->_pid_usage_info.percent, _pid_info->_pid_usage_info.utime,
           _pid_info->_pid_usage_info.stime);
